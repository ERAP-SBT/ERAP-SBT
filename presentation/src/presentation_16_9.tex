...
\begin{frame}
    ...
    \note[item]{Rekursives parsen}
    \note[item]{Start bei ELF Einstiegspunkt}
    \note[item]{RISC-V Instruktionen sequentiell in IR Instruktionen übertragen}
    \note[item]{Kontrollfluss-ändernde Instruktionen beenden Basic Blöcke}
    \note[item]{Alle möglichen Sprungziele starten neue Basic Blöcke}
    \note[item]{Diese werden rekursiv weiter geliftet}
    \note[item]{Am Ende gehen wir über alle nicht-geliftete Adressen und starten neue Basic Blöcke}
...
\begin{frame}
    ...
    \note[item]{Ein Basic Block muss aufgeteilt werden, wenn eine Kontrollflussoperation an eine Adresse springt, welche bereits geliftet ist und sich in diesem Basic Block befindet}
    \note[item]{Warum muss ein Basic Block geteilt werden? -> damit nach Definition der IR immer an den Beginn eines Basic Blocks gesprungen werden kann}
    \note[item]{bei Schleifen nötig -> siehe IR Beispiel vorher}
    \note[item]{Unbedingter Sprung als Verbindung (nach )}
    \note[item]{Variablen anhand Zuweisungsadresse (Adresse der Instruktion, welche sie erzeugt hat) auf die beiden Basic Blöcke aufgeteilt}
    \note[item]{Sorgt für Probleme, weil Datenstruktur verändert wird, v.a. richtige Aufteilen der Variablen, korrekter Sprung, korrekter Transfer der Cfops}
    \note[item]{Die Kontrollflussoperationen werden in den zweiten Basic Block (mit der höheren Adresse) geschoben, weil diese das Ende des Basic Blocks sind und sich damit garantiert im zweiten Befinden}
