...
\begin{frame}
    \frametitle{Lifter}{Aufteilen eines Baisc Blocks}
    \pause
    \begin{itemize}
        \item \textbf{Aufteilen} eines Baisc Blocks an einer Adresse, wenn in den Block an die Adresse gesprungen wird
        \pause
        \item Warum? \pause $\rightarrow$ Sprung nur an den Beginn eines Basic Blocks
        \pause
        \item Verbindung der beiden Blöcke mit einem unbedingten Sprung
        \pause
        \item Variablen werden anhand ihrer Zuweisungsadresse aufgeteilt
        \pause
        \item Kontrollflussoperationen werden in den zweiten Basic Block geschoben
        \pause
        \item Hat für große Probleme gesorgt, da bestehende Datenstruktur verändert werden muss
    \end{itemize}
\end{frame}
\clearpage
