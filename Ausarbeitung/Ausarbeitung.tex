...
The generator has two jobs. % TODO: jobs isn't the right word...
...
\section{Floating point handling}

% Bullet points:
% - more registers -> extra statics, extra mapping space
% - can be deactivated with flag
% - accuracy:
%   - known problem when dealing with different fp implementations
%   - RISC-V rounding only respected when fp -> integer conversion
% - usage of f32 and f64 IR types and specific fp operations (e.g. fma)
% - no support for exception and according flags in fcsr


% generator stuff
%   - problems with optimizations: e.g. usage of fma3, different results as with mul + add
%   - bitwise implementation too slow (e.g. qemu)

To support the \texttt{F} and \texttt{D} standard extensions, the mapping as well as the amount of
statics has to be extended to include the floating point registers and the floating point control
and status register (fcsr). To lift the RISC-V floating point, some operations are reused, e.g.
\emph{add}, \emph{sub}, and some floating point specific are used, e.g. \emph{fmul}, \emph{fsqrt}.
The variables have the types \emph{f32} and \emph{f64} which represents single and double precision
numbers.
...
rounding effects will lead to slightly different results. As we want to use the x86\_64 floating
point instructions to provide an acceptable performance, some compromises have to be made.
...
feature, it is quite hard to implement this in a fast way, because the result is already rounded and
therefore it isn't possible to round the result of each operation afterwards. And also this would be the
opposite of fast.

\par

Also a bitwise correct implementation as provided by the dynamic translator \texttt{QEMU-user} is
challenging to implement and pretty slow. So, we decided to ignore the rounding mode at lifting any
instruction except conversions from floating points to integers. This is as it makes a great
difference when rounding to integers. For example, the difference when rounding \emph{25.5} towards
positive infinity or towards negative infinity, resulting in 26 respectively 25.

\par

There are two ways of implementing this rounding. The first one is to use the ``roundss''
instruction (respectively ``roundsd'' for double precision) and the according rounding mode as
input. But these two instructions are part of SSE4.1 which isn't part of any today used CPU.\cite{intel2017man}
Therefore the usage of SSE4.1 instructions can be enabled using the optimize command line flag.

\par

The second way that is used if SSE4.1 isn't available is to change the rounding mode in the
\emph{MXCSR} register. The x86\_64 convert instructions then round accordingly. But the disadvantage
of this method is that the only way to access the MXCSR register is using ``STMXCSR'' and ``LDMXCSR''
instructions which only operates on memory operands and therefore is slower.\cite{itnel2017man}
