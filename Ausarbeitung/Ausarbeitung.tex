...
code size (C) and for control and status registers (Ziscr). They are called together with RV64 Base
ISA \texttt{rv64imacfd} or short \texttt{rv64g}. More standard extensions are planned.~\cite{rvspec}
...
registers (fcsr). The instruction length is fixed to 32bit, with the C extension to 16bit.
...
\begin{figure}
    \begin{center}
        \begin{tabular}{||c | c | c||} 
            ...
            \hline
        \end{tabular}
        \caption{Types defined for the IR variables.}\label{ir:types:figure}
    \end{center}
\end{figure}
...
instructions defining arithmetic, logical and other operations which are normally executed using a
CPU. Variables and their operations are grouped into basic blocks. These are blocks of code
which do not contain a control flow changing operation. These special operations are jumps and
system calls. The control flow changing operations act as connectors between basic blocks.
...
The IR produced by the lifter is not always optimal. For example, at the time of lifting, the lifter is not able to know
whether a variable will be used elsewhere, or whether some operation can be directly calculated or combined with another
operation that follows later. For these purposes, separate optimization passes are performed once the full IR is
available.
...
\end{lstlisting}
\caption{Example IR: The value in parenthesis after each variable is its reference count}\label{fig:dce_example_before}
\end{figure}
...
\end{lstlisting}
\caption{Example IR after Dead Code Elimination}\label{fig:dce_example_after}
\end{figure}
...
\end{lstlisting}
\caption{Example IR}\label{fig:const_folding_example_before}
\end{figure}
...
(similar to how this optimization is described in the SSA proposal), where only relevant properties, i.e. operation
type, input variables, rounding mode, or in the case of immediate variables the value itself are considered and
compared.
...
\end{lstlisting}
\caption{Example IR}\label{fig:cse_example_before}
\end{figure}
...
Figures~\ref{fig:cse_example_before} and \ref{fig:cse_example_after} show an example IR before and after applying Common
Subexpression Elimination. The first four variables visited (\texttt{v0}, \texttt{v1}, \texttt{v2}, and \texttt{v3})
are stored in the hash set, since there are no previous identical variables. The next variable, \texttt{v4}, is
the same as \texttt{v2}, so it is removed and replaced with \texttt{v2}. Then, after applying this replacement to
\texttt{v5}, it is identical to \texttt{v3}, so it is also removed. Finally, the second parameter of \texttt{v6} is
changed from \texttt{v5} to \texttt{v3}.
