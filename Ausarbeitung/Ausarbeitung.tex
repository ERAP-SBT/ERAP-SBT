...
conversion to unsigned integers is emulated by converting to a signed integer. If the input floating
...
However, the iInteger suite includes tests that not only cover integer instructions,
...
video compression, artificial intelligence and others. This allows us to stress
...
total execution time varies widely, all results are normalised to a native
...
In some benchmarks SBT is faster by a large margin, and in some other cases it performs similiarly, being slower in some cases.
Because of a bug in the floating point operatins a toolchain without hard floating point support had to be used for the \emph{625.x264} benchmark for RIA-JIT,
in pratice this could mean worse performance.

\par

% TODO: we need a hard break here to show that we go from presenting the results to interpretering their meaning.
% - Compare our results to ria-jit paper results => QEMU got faster
% - Point out that x264 is very slow compared to native
% - on the other hand with omnetpp and xz we are better than 2x native
% explain why binary translation does so exceptionally bad at x264 (video), maybe vectorisation, cross reference ria-jit paper ?
    ...
    \begin{centering}
        \begin{tikzpicture}
            \begin{axis}[
                ...
                \addplot[fill=pink]    table[x=benchmark, y expr=\thisrow{reg_alloc_plus_gen}   /\thisrow{native}] from {./tikz_src/benchmarks_results1.txt};
