...
In order to natively execute machine code, one needs a processor which runs this machine code. But there are
many different processor families with different instruction set architectures (ISA) for which programs are
developed. For running a program which is written for an ISA one does not have access to a
system running this ISA, e.g. RISC-V, one can translate it and execute it on another system,
e.g.\ x86\_64. There are three main approaches:

\par

The first option for executing programs written for a different ISA is to use an emulator which ``[\ldots] interprets program instructions at
runtime.''~\cite{binary_translation} The development is simple (Section~\ref{interpreter}), but it is a rather slow method.
...
As the third option, one can translate the full binary to an ISA which the target system is able to execute.
Translation and execution are separated, which means the binary only has to be translated once for
...
extensions to support general purpose software development''~\cite{rvspec}. There is also the
...
implementations.''~\cite{rvspec} As the name suggests, the RISC-V ISA describes a \emph{Reduced
...
up to 64-bit wide.~\cite{intel2017man} RISC-V base instructions are currently always
32-bit wide (16-bit for compressed instructions)~\cite{rvspec} which makes loading big
immediates difficult. These are either combined by two or more separate immediate loading operations
or loaded relative to the current instruction pointer.~\cite{rvspec} The latter is also often
used for performing jumps.\cite{rvspec}
...
As explained in Section~\ref{approach_section}, we use an IR to create a platform-independent
...
block with corresponding static mappers. This tells the code generator (Section~\ref{generator}) which SSA variables
...
Figure \ref{fig:lifting_example_riscv} shows an example RISC-V program and Figure
\ref{fig:lifting_example_ir} shows the resulting IR parts. For simplicity, some parts are not shown in
their entirety if they are duplicated or unnecessary for the explanation.
...
0xa8:       sll a1, a1, a2
            [...]
    \end{lstlisting}
    \caption{Example of lifting a short sequence of RISC-V instructions}
...
} => [(cjump, [b1, /* outputs */], v35, v36, v37), (jump, [b2, /* output */], v38)]
...
[...]
    \end{lstlisting}
    \caption{The lifted IR instructions}
    \label{fig:lifting_example_ir}
\end{figure}

\par

As explained in the IR chapter (Section~\ref{IR}), our IR is subdivided into basic blocks which contain
...
%\texttt{ADDI x0, x0, 0}.\cite{rvspec} The addition with zero does not change the value, and all occurrences of
...
% TODO: move to performance section ?
...
\section{Floating Point Handling and Problems}
...
The code generator (Section~\ref{generator}) uses the x86\_64 floating point instructions provided by the
...
Also the the later on used benchmarks (Section~\ref{performance}) require to respect the rounding mode of
...
%                \legend{native, qemu, test1, test2, ria}
                \legend{native,qemu,ria,master, test3}
            \end{axis}
        \end{tikzpicture}
        \caption{Benchmark Results}\label{benchmark_results1}
    \end{centering}
\end{figure}
