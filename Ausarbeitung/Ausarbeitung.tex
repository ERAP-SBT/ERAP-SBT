...
\ref{fig:lifting_example_ir} shows the resulting IR parts. For simplicity some parts aren't shown in
it's entirety if they are duplicated or unnecessary for the explanation.
...
This simple IR which decrements a counter until it's 0 produces the assembly shown in figure~\ref{fig:naive_gen_asm}.
...
\begin{enumerate}
    ...
    \item To reduce this size overhead, we implemented a method for creating a nearly perfect hash table with a
          ...
          indices table size.\ref{hashing_figure} The storage size optimizations mean however that
...
is an non optimized emulator.\footnote{Our emulator is called ``interpreter'' in the implementation
    and therefore we use this naming here also.} So the interpreter is pretty slow as it decodes the
...
We didn't optimized the interpreter much because it is only a fall back system for the translated
...
The interpreter is called if an indirect jump or indirect call target wasn't resolved and therefore
the generator doesn't know where to jump. So the interpreter is used to fill this gap and to jump
...
the naive generator (\ref{naive_generator}) takes the basic blocks inputs there from the statics.
When using the register allocator (\ref{reg_alloc}) the translation blocks
(\ref{translation_blocks}) move the values to the correct registers and stack slots and are
therefore - as well as the top level basic blocks - the addresses where we can leave the
interpreter. These addresses are stored in the ijump lookup table (\ref{ijump_resolution}).
...
extensions as the translator. If the ``--interpreter-only'' flag is supplied to the translator no
...
slow and therefore isn't desirable to use as translate binaries.
