...
% TODO: Moved from lifter general method chapter, needs to be integrated here 
We introduce a \emph{memory token} which is a required input for operations which read from memory and the
result of operations which write to memory. It creates a dependency between storing and loading
operations. This should be used to keep the memory access operations
in their correct order when reordering operations inside basic blocks and functions. A writing
operations acts as a separator of reading operations, where reading operations can be swapped freely
with each other. This insures a valid memory access ordering and therefore the correctness of the
program.
% 
...
The ELF-file loader parses instructions from the \emph{SHT\_PROGBITS}-type sections of the binary
which are marked with the flags
\emph{SHF\_ALLOC} and \emph{SHF\_EXECINSTR}. The flags mark that the section ``occupies memory during
...
Due the memory architecture of modern computers, instructions and data are mostly stored in the same
...
instructions which are disassembled using frvdec.\cite{frvdec} The result of this disassembly is a
more generic and easier processable form, e.g. direct access to the input and output registers and
immediate values. For each of the instructions a sequence of variables and operations is added to
the IR. This sequence depicts the functionality of the instruction.
% TODO: Add example for lifting an instruction here

\par

As explained in the IR chapter (\ref{IR}) our IR is subdivided into basic blocks which depicts
consecutive instructions without any control flow change. When a control flow changing instruction
is discovered, it is added to the current basic block as well as the variables needed for this
instruction, e.g. calculating the jump address. If the jump address can be inferred from the
instruction, in particular considering \emph{jal} and branch instructions where the jump address is
the sum of the address of the instruction and an offset, this address is used to define the start of
a new basic block. In the case that the instruction at the jump address isn't processed yet, the
lifter remembers that at this address a basic block must start. But in the other case, it needs to
be checked whether at this address a basic block already starts and otherwise the block containing
the jump address must be split up at the jump address. This is required because it is only possible
to jump to the start of an basic block.
...
Using the command line flag ``--full-backtracking'' the user can control whether all possible
address should be found or the first one is used. If no jump address can be inferred, a dummy
basic block which indicates an unresolved jump target, is set as target block. The jump address then
needs to be evaluated at runtime and jumps to an address at which no basic block begins needs to be
...
which represents the content of the registers. This is required to determine the inputs of the
operations based on the input registers of the instructions. For example, when lifting an RISC-V
instruction which has the registers \emph{x13} and \emph{x14} as inputs, the input variables are
taken from the mapping at the corresponding indices.

\par

At the start of a basic block the mapping is initialized
with variables from \nameref{statics}. The register with the index zero is hard wired zero in
...
variable with value zero. Each write to the zero register is ignored, as it is specified for the \emph{x0}
register.\cite{rvspec}

\subsubsection{Postprocessing}

After all instructions are processed, the jump targets of the basic blocks which jump to a later
created basic block needs to be assigned. Also the predecessor and successor lists of the involved
basic blocks are fixed.

\par

The last step in lifting is to add a basis block which setups the RISC-V stack and then jumps to the
basic block which starts at the entry address. This address is stored in the program header
\emph{e\_entry} and specifies where the execution of the binary starts.\cite{man_elf}
