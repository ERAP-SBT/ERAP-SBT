...
For mapping the RISC-V indirect jump target addresses to translated basic blocks, we implemented two
different methods:
...
hash functions. It is also implemented in the \nameref{helper} for providing hashes for runtime
addresses. This helps reduce the required storage space to the hash table size (which is the amount
of basic blocks divided by the load factor) and the hash function indices table size (which is the
hash table size divided by the amount of buckets).
\end{itemize}
