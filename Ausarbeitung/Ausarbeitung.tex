...
typing, SSA variables (\ref{ssa}) and machine-instruction-like operations. 
...
and one control and status flag register. The register \emph{x0} is always zero and thus not
...
mappers usually have the standard size and type of the static's associated register, e.g. \emph{i64} for
general purpose registers (statics 0 to 31) and \emph{f64} for floating point registers (statics 33
...
instruction can be assembled of more CfOps, e.g. RISC-V branches which consists of a conditional
...
\nameref{backtracking} or it is directly encoded, like in branch instructions, this address is used
...
the instructions. For example, when lifting an RISC-V instruction which has the registers \emph{x13}
and \emph{x14} as inputs, the input variables are taken from the mapping at the corresponding
indices.
