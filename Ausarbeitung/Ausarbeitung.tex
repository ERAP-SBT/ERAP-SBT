...
target address, we switch to the \nameref{interpreter} to interpret the original RISC-V code
...
\item To reduce this overhead, we implemented a method for creating a nearly perfect hash table with a
...
factor of 0.95, which is lowered incrementally by 10\% if no valid hash function could be found.
We also use an average bucket size of 3 elements which was identified as a good bucket size in the
referenced paper. The bucket-level hash functions only need a 16-bit index to be reconstructed and can
thus
