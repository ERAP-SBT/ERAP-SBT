...
We also introduced a \emph{memory token} which is always a input for \emph{load} operations and the
result of \emph{store} operations. The main reason to use this extra variable is to keep track of
memory operations because they need to be kept in order when performing operations. This means that
\emph{load} operations can be mixed up as needed as long as no \emph{store} operation is in between.
But \emph{store} operations cannot be swapped with other \emph{stores} or \emph{loads}. This insures
that the correct values are read from and written to memory and therefore the correctness of the
program. So the memory token creates dependencies between the memory operations. The second reason
is that our IR assigns each operation to the variable which is the result of this operation, but a
\emph{store} operation has no result except the change of the memory. The memory token is also
