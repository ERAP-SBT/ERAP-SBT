...
instructions which are decoded using frvdec.\cite{frvdec} The resulting instructions are c-structs,
...
the IR to replicate the origin program's behavior.
...
consecutive instructions without a change in control flow. As soon as a control flow changing instruction
is discovered, is is added to a special list of CfOps and the basic block is finished. 
If the jump address can be inferred from the
instruction using \nameref{backtracking} or it is directly encoded, like in branch instructions, 
this address is used to register the start of
a new basic block. 
...
sorted into the first block, all others into the second. Both blocks are connected using a
direct jump. 
...
which represents the content of all integer and floating point registers. This is required to
determine the inputs of the
...
The last step in lifting is to add an entry basis block which sets the RISC-V stack up and jumps to the
basic block at the ELF-file's entry address. This virtual address is stored in the ELF program header
\emph{e\_entry} and specifies where the execution of the binary starts.\cite[p.~5]{elf_spec}
