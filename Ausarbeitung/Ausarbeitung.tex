...
program loader information, stored in the \emph{program header table}. The stored program headers
...
While lifting the instructions a register file, called \emph{mapping}, emulates changes to RISC-V
registers and stores their state. This is required for tracking register contents between
instructions. At the start of a basic block the mapping is initialized
...
variable with value zero. Each write to the zero register as ignored, as it is specified for the \emph{x0}
register.\cite{rvspec}

\subsection{Memory tokens}

% TODO: is this paragraph part of IR or Lifter?
...
When a control flow changing instruction is discovered, it is added to the current basic block as
well as the variables needed for this instruction, e.g. calculating the jump address. If the jump
address can be inferred from the instruction, in particular considering \emph{jal} and branch
instructions where the jump address is the sum of the address of the instruction and an offset, this
address can be used to define the start of a new basic block. If the instruction at
the jump address isn't processed yet, the lifter remembers that at this address is the start of a
basic block. But in the other case, it needs to be checked whether at this address a basic block starts and
otherwise this block must be split up at the jump address.
...
address should be found or the first one found is used. If no jump address can be inferred, a dummy
basic block which indicates an unresolved CfOp, is set as target block. The jump address then needs
to be evaluated at runtime and jumps to an address at which no basic block begins needs to be
handled by the helper library (\ref{helper}).
