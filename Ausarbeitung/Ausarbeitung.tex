...
When encountering an indirect jump, the input variable contains the jump target address in the
original RISC-V binary, at runtime. We try to statically identify most of the indirect jump target
addresses during lifting and are thus able to start new basic blocks at such addresses. 
This means we need a helper function which translates RISC-V
addresses into basic block addresses in the generated assembly. We also need a way to identify
target addresses which aren't the start address of any existing basic block. This situation is
handled by a panic function, defined in the \nameref{helper}. It prints a supplied panic string to
the standard error output and exits the program with a non-zero exit code. 

\par 

Mapping the RISC-V indirect jump target addresses to translated basic blocks requires a dictionary.
The implementation has to have exceptionally fast access times as we need to access entries with
every indirect jump. It also has to be able to identify invalid keys, to escalate a panic if the
jump target address is not a valid basic block start address. Therefore, we implemented two
different methods:

\begin{itemize}
\item For the naive implementation, we store a 64-bit wide number for each RISC-V address, after
some label in the \texttt{.ro\_data} section. This recreates part of the original binary's address
...
numbers.
\par
This method has the advantage of having a constant access time with no calculation overhead. It
...
\item To reduce this overhead, we implemented a method for creating a perfect hash table with a
...
bucket. The hashed values are stored in one hash table with a load factor of 0.95 and an average bucket size
of 3 elements. The bucket-level hash functions only need a 16-bit index to be reconstructed and can thus
...
function''\cite{CHD}, on of Bob Jenkins' first published hash functions.\cite{jenkins_hash_1} For
...
don't need most of his hash function which generates hashes for byte arrays of variable sizes. 
...
% TODO: provide examples for assembly size reduction
