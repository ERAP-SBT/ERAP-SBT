...
void Generator::compile_block_reg_alloc(const BasicBlock *bb) {
    ...
    const auto load_val_in_reg = [&reg_map, &var_map, bb, this, &alloc_reg, &clear_reg](SSAVar *var, size_t cur_time, REGISTER only_this_reg = REG_NONE,
                                                                                        REGISTER keep_clear_reg = REG_NONE) -> REGISTER {
        const auto idx = index_for_var(bb, var);
        ...
        if (var_map[idx].cur_reg != REG_NONE && var_map[idx].cur_reg == keep_clear_reg) {
            // var is in reg that needs to stay clear
            fprintf(out_fd, "mov %s, %s\n", reg_names[reg][0], reg_names[var_map[idx].cur_reg][0]);
            reg_map[var_map[idx].cur_reg].current_var = nullptr;
    ...
    for (size_t i = 0; i < bb->variables.size(); ++i) {
        ...
        case Instruction::sar: {
            ...
            if (in1->type == Type::imm && in2->type == Type::imm && !std::get<SSAVar::ImmInfo>(in1->info).binary_relative && !std::get<SSAVar::ImmInfo>(in2->info).binary_relative) {
                // space for dst
                ...
                if (in1->type == Type::imm) {
                    if (op->type != Instruction::sub && op->type != Instruction::sumul_h && op->type != Instruction::div && op->type != Instruction::udiv && op->type != Instruction::shl &&
                        op->type != Instruction::shr && op->type != Instruction::sar) {
                ...
                // TODO: this doesnt handle ssmul and uumul, whoops
                assert(op->type != Instruction::ssmul_h && op->type != Instruction::uumul_h);
